...
temperatures = data_slice['Tu (K)'].values[0]
pressures = data_slice['Pu (atm)'].values[0] * ct.one_atm
...
def run_flame_speed(condition_index):
    ...
    csv_filepath_guess = os.path.join(initial_guess_path, f"flame_{condition_index}.csv")
    if os.path.exists(csv_filepath_guess):
        print('Loading initial flame conditions from', csv_filepath_guess)
        df = pd.read_csv(csv_filepath_guess)

        df_pruned = df.iloc[::2, :24]  # I've already forgotten why it's :24?
        print(df)
        flame.set_initial_guess(data=df_pruned)
    else:
