...
# modify this to do 16 at a time because Aramco is SLOW
import os
...
def run_flame_speed(condition_index):
    ...
    initial_guess_path = os.path.join(this_dir, mechs[os.path.basename(cti_path)[:-4]])
    h5_filepath_guess = os.path.join(initial_guess_path, f"saved_flame_{condition_index}.h5")
    ...
    # print("Save YAML")
    # yaml_filepath = os.path.join(save_dir, f"saved_flame_{condition_index}.yaml")
...
condition_indices = np.arange(0, 16)
if fs_set == 1:
    condition_indices = np.arange(16, 32)
elif fs_set == 2:
    condition_indices = np.arange(32, 48)
elif fs_set == 3:
    condition_indices = np.arange(48, 51)
...
# out_df.to_csv(f'{os.path.join(save_dir, os.path.basename(cti_path))[:-4]}.csv')
