...
for i in range(rxn_index_start, min(rxn_index_start + REACTIONS_PER_FILE, len(base_gas.reactions()))):
    print(f'perturbing {i} {base_gas.reactions()[i]}')

    # load the base gas
    # base_gas = ct.Solution(base_yaml_path)
    base_gas.set_multiplier(1.1, i)
    ...
    base_gas.set_multiplier(1.0, i)  # return gas back to normal state
