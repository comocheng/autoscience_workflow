...
def run_flame_speed(condition_index):
    ...
    h5_filepath_guess = os.path.join(flame_speed_dir, f"saved_sp_flame_{condition_index}.h5")
    ...
    hdf_filepath = os.path.join(flame_speed_dir, f"saved_sp_flame_{condition_index}.h5")
    flame.write_hdf(
        hdf_filepath,
        group="freeflame",
        mode="w",
...
species_flame_speeds = np.zeros((len(perturbed_gas.species()), len(equiv_ratios)))

print('Going through species...')
# do one npy per species
for i in range(0, len(perturbed_gas.species())):
    species_npy = os.path.join(flame_speed_dir, f'sp_{i:04}.npy')
    if os.path.exists(species_npy):
        print(f'skipping {i} {perturbed_gas.species()[i]} file already exists')
        species_flame_speeds[i, :] = np.load(species_npy)
        continue
    ...
    species_flame_speeds[i, :] = flame_speeds

    # # do in series first for debugging
    # for condition_index in condition_indices:
    #     flame_speeds[condition_index] = run_flame_speed(condition_index)
    # species_flame_speeds[i, :] = flame_speeds

    np.save(species_npy, flame_speeds)

# save the result as a pandas dataframe
np.save(os.path.join(flame_speed_dir, f'species_flame_speeds.npy'), species_flame_speeds)
