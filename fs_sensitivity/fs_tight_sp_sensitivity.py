...
def run_flame_speed(condition_index, param_index):
    ...
    hdf_filepath = os.path.join(param_dir, f"perturbed_flame_tight_sp_{scaled_index}.h5")
    flame.write_hdf(
        hdf_filepath,
        group="freeflame",
        mode="w",
...
for i in range(sp_index_start, min(sp_index_start + 50, len(perturbed_gas.reactions()))):
...
np.save(os.path.join(f'flame_speeds_tight_sp_{sp_index_start:04}.npy'), reaction_flamespeeds)
