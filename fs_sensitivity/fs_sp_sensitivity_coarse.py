...
chemkin = sys.argv[1]
sp_index_start = int(sys.argv[2])  # start index for SLURM parallelization
...
def run_flame_speed(condition_index, param_index):
    ...
    # Too many to run to save h5 files? nah
    print("Save HDF")
    param_dir = os.path.join(working_dir, f'param_{param_index:04}')
    os.makedirs(param_dir, exist_ok=True)
    hdf_filepath = os.path.join(param_dir, f"perturbed_flame_coarse_sp_{scaled_index}.h5")
    flame.write_hdf(
        hdf_filepath,
        group="freeflame",
        mode="w",
...
for i in range(sp_index_start, min(sp_index_start + 50, len(perturbed_gas.species()))):
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=MAX_WORKERS) as executor:
        for condition_index, flame_speed in zip(condition_indices, executor.map(
            run_flame_speed,
            condition_indices,
            [i for x in condition_indices])
        ):
            flame_speeds[condition_index] = flame_speed
    reaction_flamespeeds[i, :] = flame_speeds

np.save(os.path.join(f'flame_speeds_coarse_sp_{sp_index_start:04}.npy'), reaction_flamespeeds)
