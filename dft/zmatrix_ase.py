...
def get_atom_ranks(atoms, cutoff_multiplier=1.0):
    ...
    for i, atom in enumerate(atoms):
        for neighbor_index in geo_analysis.all_bonds[0][i]:
            sum_nearest_neighbor_bonds[i] += len(geo_analysis.all_bonds[0][neighbor_index])

    ratings = np.array(atomic_numbers) + np.array(n_bonds) + np.array(sum_nearest_neighbor_bonds)
    sorted_ratings = sorted(ratings)
    rank = []
    for i in range(len(atoms)):
        rank.append(sorted_ratings.index(ratings[i]))
    return rank
...
class ZMatrix(object):
    ...
    def get_neighbor_idxs(self, atm_idx):
        atm = self.atoms[atm_idx]
        idx_rank_list = list()
        for atm_nghbr_idx in self.analysis.all_bonds[0][atm_idx]:
            idx_rank_list.append([self.rank[atm_nghbr_idx],
                                 atm_nghbr_idx])
            ...
            for intersect_1 in itertools.permutations(intersect, path_length - 1):
                ...
                for i in range(1, path_length):
                    for atm_intersect in intersect_1:
                        ...
                        if not (l1 == i and l2 == (path_length - i)):
                            continue
                        if self.is_neighbor_of(atm_intersect, kpaths[i - 1]):
                            kpaths[i] = atm_intersect
                maxattempts -= 1
        ...
        if self.N_atms == 1:
            ...
            for query_atm_idx in self.ordered_atom_list[:self.N_atms - 1]:
                ...
                if self.get_path_length(atm_idx, query_atm_idx) == 1:
                    ...
                    for atm_idx_tmp in self.ordered_atom_list[:self.N_atms]:
                        if atm_idx_tmp not in self.z[self.N_atms - 1]:
    ...
    def build_cart_crds(self, z_crds, virtual_bond=None, virtual_angles=None,
        ...
        if virtual_bond is None:
            virtual_bond = 1.0 * unit.nanometer
        if virtual_angles is None:
            virtual_angles = np.array([np.pi / 2.0, np.pi / 2.0]) * unit.radian
        if virtual_dihedrals is None:
            virtual_dihedrals = np.array([np.pi / 2.0, np.pi / 2.0, np.pi / 3.0]) * unit.radian
        if attach_crds is None:
